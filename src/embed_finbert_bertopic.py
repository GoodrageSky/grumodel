...
class FinBertEmbedder:
    ...
    def __init__(self, cfg: EmbedConfig = EmbedConfig()):
        ...
        if self.cfg.verbose:
            print(f"[embed] loading model={cfg.model_name} device={self.device}", flush=True)
    ...
    def encode(self, texts: list[str]) -> np.ndarray:
        ...
        vecs = []
        total = len(texts)
        n_batches = (total + self.cfg.batch_size - 1) // self.cfg.batch_size
        if self.cfg.verbose:
            print(
                f"[embed] encoding texts={total} batch_size={self.cfg.batch_size} "
                f"batches={n_batches}",
                flush=True,
            )
        for i in range(0, len(texts), self.cfg.batch_size):
            ...
            done_batches = i // self.cfg.batch_size + 1
            if self.cfg.verbose and (
                done_batches == 1
                or done_batches == n_batches
                or done_batches % max(self.cfg.progress_every, 1) == 0
            ):
                done = min(i + self.cfg.batch_size, total)
                print(
                    f"[embed] completed {done}/{total} "
                    f"({done / max(total, 1):.1%})",
                    flush=True,
                )
    ...
    def fit(self, embeddings_train: np.ndarray, y_train: np.ndarray,
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Missing dependency 'umap'. Install it with: "
        ...
        if self.cfg.use_bertopic and docs_train is not None:
            try:
                ...
                raise ModuleNotFoundError(
                    "Missing dependency 'bertopic'. Install it with: "
