...
# Set before running:  RDS_HOST / RDS_USER / RDS_PASSWORD  (RDS_PORT optional)
import os

DB_HOST = os.environ.get("RDS_HOST", "quantstudio.mysql.rds.aliyuncs.com")
DB_USER = os.environ.get("RDS_USER", "gjzq")
...
def _connect(database: str) -> pymysql.connections.Connection:
    if not DB_PASS:
        raise RuntimeError(
            "RDS_PASSWORD environment variable is not set — required to connect "
            "to the research-report database.")
    return pymysql.connect(host=DB_HOST, user=DB_USER, password=DB_PASS,
                           database=database, port=DB_PORT, charset="utf8mb4")
