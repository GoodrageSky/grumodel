...
gru_lgbm_factor.py
==================
Stage 3 of the daily-frequency text-factor pipeline: turn a panel of
...
1.  DAILY-FREQUENCY AWARE sequence builder. Windows are only built when the
...
def compute_ic(pred: pd.Series, df: pd.DataFrame, label_col: str, date_col: str = "date"):
    ...
    return {
        ...
        "n_days": len(ic), "IC_series": ic, "RankIC_series": ric,
    }


def quantile_backtest(pred: pd.Series, df: pd.DataFrame, ret_col: str, n_q: int = 5):
    """Top-minus-bottom quantile daily long-short return curve. Needs a raw
    ...
    daily_ls = tmp.groupby(lvl).apply(_ls).dropna()
    return {"ls_daily": daily_ls, "ls_mean": daily_ls.mean(),
            "ls_sharpe": daily_ls.mean() / (daily_ls.std() + 1e-12) * math.sqrt(252),
            "ls_cum": (1 + daily_ls).cumprod()}
...
def run_pipeline(df: pd.DataFrame, segments: dict, cfg: FactorConfig):
    ...
    for name, p in preds.items():
        ...
        print(f"[{name}] IC={m['IC_mean']:.4f} RankIC={m['RankIC_mean']:.4f} "
              f"ICIR={m['ICIR']:.3f} (n_days={m['n_days']})")

    bt = None
    if cfg.return_col and cfg.return_col in df_gru.columns:
        bt = quantile_backtest(preds["test"], df_gru, cfg.return_col)
